...
async function run() {
  ...
  const { employee: hr } = await createUserAndEmployee({
    name: 'Priya Nair', email: 'hr@nimbuslabs.io', role: 'hr', designation: 'HR Manager',
  ...
  // --- Office floor + desks, so the Workforce Command Center (Phase 10)
  // has a real map to show instead of an empty grid --------------------
  log('Setting up floor plan and desk assignments...');
  const floor1 = await Floor.create({ office: office._id, name: 'Floor 1 — Engineering', floorNumber: 1, gridWidth: 10, gridHeight: 6 });
  const floor2 = await Floor.create({ office: office._id, name: 'Floor 2 — Product & Ops', floorNumber: 2, gridWidth: 10, gridHeight: 6 });

  // Everyone who should show up on the map: the 3 managers plus every
  // seeded employee. Engineering (platform + frontend) sits on Floor 1;
  // Product, HR, and Admin sit on Floor 2 - laid out in a simple grid,
  // leaving a few empty desks on each floor for realism.
  const floor1People = [mgr1, mgr2, ...employees.filter((e) => [platformTeam._id.toString(), frontendTeam._id.toString()].includes(e.team._id.toString())).map((e) => e.employee)];
  const floor2People = [mgr3, admin, hr, ...employees.filter((e) => e.team._id.toString() === productTeam._id.toString()).map((e) => e.employee)];

  const seatFloor = async (floor, people) => {
    const cols = floor.gridWidth - 2;
    for (let i = 0; i < people.length; i += 1) {
      const x = 1 + (i % cols);
      const y = 1 + Math.floor(i / cols);
      const desk = await Desk.create({ floor: floor._id, deskCode: `${floor.floorNumber}-${String(i + 1).padStart(2, '0')}`, position: { x, y }, assignedEmployee: people[i]._id });
      await Employee.findByIdAndUpdate(people[i]._id, { desk: desk._id, floor: floor._id, office: office._id });
    }
    // A few extra unassigned desks per floor, so the map shows some open seats too.
    for (let i = 0; i < 3; i += 1) {
      const idx = people.length + i;
      const x = 1 + (idx % cols);
      const y = 1 + Math.floor(idx / cols);
      await Desk.create({ floor: floor._id, deskCode: `${floor.floorNumber}-${String(idx + 1).padStart(2, '0')}`, position: { x, y } });
    }
  };
  await seatFloor(floor1, floor1People);
  await seatFloor(floor2, floor2People);
